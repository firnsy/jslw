...
Slider = function(p, r, c)
{
  ...
  this.add_event_listener('mouse_down', function(slider) {slider.set_dirty(true);});
  this.add_event_listener('mouse_drag_end',   function(slider) {slider.set_dirty(true);});
  this.add_event_listener('mouse_drag_move', function(slider) {slider.set_dirty(true);});
}
 ...
 */
Slider.prototype.set_level = function(l)
{
  if (this.is_drag) return; // no update while setting
  if (l < 0 ) l = 0;
  if (l > 100) l = 100;
 ...
 */
Slider.prototype.set_margins = function(m)
{
  this.margins = m;
}
/**
 ...
 * level is changed
 * 
 * @TODO still want to call the widget base class for any other events
 */
Slider.prototype.add_event_listener = function(a, cb)
{
  ...
  switch(a)
  {
    ...
    case "mouse_drag_move":
      ...
      // calc % change
      slider.level = slider.level + slider.calc_slider_level();
      if (slider.level < 0 ) slider.level = 0;
      if (slider.level > 100) slider.level = 100;
      slider.set_dirty(true);
 ...
 */
Slider.prototype.calc_slider_level = function()
{
 ...
 */
Slider.prototype.set_background_image_up = function(path)
{
  ...
  this.background_image_up.onload = function() { this_object.set_dirty(true); };
 ...
 */
Slider.prototype.set_background_image_down = function(path)
{
  ...
  var this_object = this;
  this.background_image_down.onload = function() { this_object.set_dirty(true); };
}
/**
 * Sets the on pressed image
 */
Slider.prototype.set_level_image = function(path)
{
  ...
  var this_object = this;
  this.level_image.onload = function() { this_object.set_dirty(true); };
}
/**
 * Main rendering fn
 */
Slider.prototype.render_widget = function(context)
{
  // draw the fill color
      ...
      this.level > 0 ) 
  {
    ...
    //console.log('level slider X:' + xorig + ' Y:' + yorig + ' W:' + width + ' H:'+ height + ' BH:' + this.bounds.h);
